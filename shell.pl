...
non_space(Code) :- \+ code_type(Code, space).
is_blank(S) :-
  string(S),
  string_codes(S, Cs),
  include(non_space, Cs, NonBlank),
  NonBlank == [].

% swallow pending input before returning to menu
...
string_trim(S0, S) :-
  string_codes(S0, Cs0),
  trim_leading(Cs0, Cs1),
  reverse(Cs1, R0),
  trim_leading(R0, R1),
  reverse(R1, Cs),
  string_codes(S, Cs).
trim_leading([C|R], Out) :-
  code_type(C, space), !, trim_leading(R, Out).
trim_leading(L, L).
...
% Resolve a user-typed title to the canonical KB title (case-insensitive).
...
% Swallow empty/whitespace choices quietly
...
% -------- Recommend (with filters + breakdown) ----------
cmd_recommend :-
...
find_and_display_recs(Prefs, AgeOpt, BucketOpt) :-
  ...
  ; nl, format('--- Recommendations ---~n'),
    forall(member(T, Titles2),
      ( print_card(T),
        explain_breakdown(Prefs, T)
      )),
    format('--------------------------------~n')
  ).
...
cmd_search :-
  ...
  ( L = [] ->
      ...
      press_enter_to_continue
    ; nl, format('--- Matches ---~n'),
      print_indexed_list(L),
      format('---------------~n'),
      prompt_yes_no('Add any to watchlist? (y/n): ', YN),
      ( YN = yes ->
          choose_title_from(L, TPicked),
          add_to_watchlist(TPicked),
          format('Added.~n')
        ; true ),
      press_enter_to_continue
  ).
...
cmd_watchlist :-
  ...
  ( C="a" ->
      ...
      ( add_to_watchlist(T0) -> format('Added.~n')
      ; format('Not found in KB; not added.~n') ),
      press_enter_to_continue
  ; C="r" ->
      format('Title to remove (case-insensitive): '),
      read_line_to_string(user_input,T0),
      ( resolve_title(T0, T), retractall(watchlist(T)) -> format('Removed.~n')
      ; format('Not on watchlist.~n') ),
      press_enter_to_continue
  ; C="s" ->
      ...
      press_enter_to_continue
  ; C="c" ->
      retractall(watchlist(_)), format('Watchlist cleared.~n'),
      press_enter_to_continue
  ; format('Unknown option.~n'),
    press_enter_to_continue
  ).

% ===================================================================
% --- 5. INPUT / PRINT HELPERS
% ===================================================================
...
ask_age_filter(Out) :-
  ...
  ( S == "" ->
      Out = none
  ; ( catch(number_string(Age, S), _, fail),
      integer(Age), Age >= 0, Age =< 120 ) ->
      Out = some(Age)
  ; format('Invalid age. Skipping age filter.~n'),
    Out = none
  ).

ask_length_filter(Out) :-
  format('Optional length bucket [short/medium/long] (blank for any):~n> '),
  read_line_to_string(user_input, B0),
  string_lower(B0, B),
  ( B == "" ->
      Out = none
  ; member(B, ["short","medium","long"]) ->
      atom_string(Bucket, B), Out = some(Bucket)
  ; format('Invalid bucket. Skipping length filter.~n'),
    Out = none
  ).
...
replace_all(S, From, To, Out) :-
  ( sub_string(S, Before, _, _, From) ->
      ...
      Start is Before + LFrom,
      string_length(S, SL),
      SuffixLen is SL - Start,
      sub_string(S, Start, SuffixLen, 0, Suffix),
      replace_all(Suffix, From, To, ReplacedSuffix),
      string_concat(Prefix, To, Temp),
      string_concat(Temp, ReplacedSuffix, Out)
  ; Out = S
  ).

to_s(X, S) :- ( string(X) -> S = X ; atom(X) -> atom_string(X,S) ; number(X) -> number_string(X,S) ; term_string(X,S) ).

% SAFE: always succeeds (falls back if KB lookup fails)
print_card(Title0) :-
